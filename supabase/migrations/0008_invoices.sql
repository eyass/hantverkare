...
-- Atomically bumps and returns the next sequence number for the CALLING user's
-- current year, formatted as the full invoice number. Race-safe: the insert ... on
-- conflict do update ... returning is a single atomic statement, so concurrent callers
-- for the same (user_id, year) serialize on the row and each gets a distinct, strictly
-- increasing sequence -- there is no read-then-write window visible to callers where two
-- could observe the same last_seq.
--
-- Known gap (fixed): this function used to take p_user_id as a caller-supplied argument.
-- Since Supabase exposes every public-schema function via PostgREST (/rpc/...), that let
-- any authenticated client bump/corrupt another user's counter row directly, bypassing
-- the "only touches its own row" assumption entirely. Fixed by deriving the user from
-- auth.uid() internally instead of trusting an argument -- the function now has no
-- parameters at all, so there is nothing for a caller to spoof.
