...
export function GradientBackdrop({ className = "" }: { className?: string }) {
  ...
  const blobs = [
    {
      className:
        "left-[-10%] top-[-10%] h-[420px] w-[420px] bg-[radial-gradient(circle_at_30%_30%,#93c5fd,transparent_70%)]",
    ...
    {
      className:
        "right-[-15%] top-[5%] h-[480px] w-[480px] bg-[radial-gradient(circle_at_60%_40%,#bfdbfe,transparent_70%)]",
      animate: shouldReduceMotion
        ? undefined
    ...
    {
      className:
        "left-[20%] bottom-[-20%] h-[380px] w-[380px] bg-[radial-gradient(circle_at_50%_50%,#60a5fa,transparent_70%)]",
      animate: shouldReduceMotion
        ? undefined
    ...
    >
      {blobs.map((blob, index) =>
        shouldReduceMotion ? (
          ...
          <div
            key={index}
            className={`absolute rounded-full opacity-50 blur-3xl ${blob.className}`}
          />
        ) : (
          <motion.div
            key={index}
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,transparent_0%,#ffffff_95%)]" />
    </div>
  );
}
