...
export function PhoneDemoHero() {
  ...
  const [phase, setPhase] = useState<
    "typing" | "generating" | "items" | "signed"
  >("typing");
  ...
  function runCycle() {
    ...
    const full =
      EXAMPLE_DESCRIPTIONS[(myCycle - 1) % EXAMPLE_DESCRIPTIONS.length];
    setPhase("typing");
    setTypedText("");
    setQuote(null);
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-[26px] bg-white text-left">
        ...
        <div className="flex justify-center px-[18px] pt-4">
          <span className="inline-flex items-center rounded-full bg-[#eef2ff] px-2.5 py-1 text-[10.5px] font-semibold tracking-wide text-blue-600 uppercase">
            {phaseLabel}
          </span>
        </div>
        <div className="px-[22px] pt-2 pb-2 text-center">
          <div className="text-[19px] font-bold text-[#0f172a]">
            {phase === "items"
              ? t.hero.itemsTitle
              : phase === "signed"
                ? t.hero.signedHeading
                : t.hero.phoneTitle}
          </div>
        </div>
        {phase === "items" || phase === "signed" ? (
          <div className="mono px-[18px] pb-1 text-center text-xs font-semibold text-blue-600">
            {statusText}
          </div>
        ) : (
          <>
            ...
            <div className="mono text-center text-xs font-semibold text-blue-600">
              {statusText}
            </div>
          </>
        )}
        ...
        {phase === "items" && quote ? (
            ...
            <div className="flex max-h-[150px] flex-col gap-1.5 overflow-y-auto pr-1">
              {quote.items.map((item, i) => (
                ...
                >
                  ...
                  <span className="mono flex-none text-[#0f172a]">
          ...
          <div className="mx-[18px] my-4 flex flex-col items-center gap-2 rounded-[13px] bg-[#f0fdf4] p-4 text-center text-[13px] text-[#166534]">
            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#22a06b]">
              <svg
                width="22"
                height="22"
                viewBox="0 0 24 24"
            ...
            <div className="mono text-base font-bold text-[#166534]">
              {formatCents(quote.totalCents)}
            </div>
            <div className="text-[11.5px] text-[#4d7c5f]">
              {t.hero.signedBy} {t.hero.signedCustomer}
            </div>
          </div>
        ) : (
