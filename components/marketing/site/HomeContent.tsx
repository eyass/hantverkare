...
export function HomeContent() {
  ...
  return (
    <div>
      {/* HERO */}
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-4 pt-14 pb-14 sm:px-8 sm:pt-16 md:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <p className="mb-7 max-w-[480px] text-lg leading-relaxed text-[#475569] sm:text-[19px]">{t.hero.sub}</p>
          ...
          <div className="text-[13.5px] text-[#64748b]">{t.hero.statLine}</div>
        </div>
      ...
      <AnimatedSection className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
        <div className="mx-auto mb-12 max-w-xl text-center">
          <h2 className="mb-3.5 text-[34px] font-bold tracking-tight text-balance text-[#0f172a]">{t.features.head}</h2>
          <p className="text-[17px] leading-relaxed text-[#64748b]">{t.features.sub}</p>
        </div>
        <div className="grid gap-5 sm:grid-cols-3">
          {t.features.items.map((f, i) => (
            ...
            >
              ...
              <div className="mb-2 text-lg font-bold text-[#0f172a]">{f.title}</div>
              <div className="text-[14.5px] leading-relaxed text-[#64748b]">{f.body}</div>
            </div>
          ))}
        </div>
      </AnimatedSection>
      ...
      <section className="border-t border-b border-[#eef1f4] bg-[#fafbfc]">
        <AnimatedSection className="mx-auto max-w-4xl px-4 py-16 sm:px-8">
          <div className="mx-auto mb-10 max-w-xl text-center">
            <h2 className="mb-3 text-[32px] font-bold tracking-tight text-balance text-[#0f172a]">{t.calc.head}</h2>
      ...
      <section className="bg-[#0f172a] text-white">
        <AnimatedSection className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
          <div className="mx-auto mb-12 max-w-xl text-center">
            <h2 className="mb-3 text-[32px] font-bold tracking-tight">{t.steps.head}</h2>
            <p className="text-[17px] text-[#94a3b8]">{t.steps.sub}</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            {t.steps.items.map((s, i) => (
              <div key={s.title} className="rounded-[18px] border border-[#334155] bg-[#1e293b] p-6">
                ...
                <div className="text-[14.5px] leading-relaxed text-[#94a3b8]">{s.body}</div>
              </div>
            ))}
          </div>
        </AnimatedSection>
      ...
      <AnimatedSection className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
        <div className="mx-auto mb-2.5 max-w-xl text-center">
          <h2 className="mb-2 text-[32px] font-bold tracking-tight text-balance text-[#0f172a]">{t.testimonials.head}</h2>
          <p className="mb-9 text-xs font-medium text-[#94a3b8]">{t.testimonials.caption}</p>
        </div>
        <div className="grid gap-5 sm:grid-cols-3">
          {t.testimonials.items.map((tm) => (
            ...
            >
              ...
              <div className="flex-1 text-[15.5px] leading-relaxed text-[#0f172a]">{tm.quote}</div>
      ...
      <section className="border-t border-[#eef1f4] bg-[#fafbfc]">
        <AnimatedSection className="mx-auto max-w-6xl px-4 py-16 sm:px-8">
          <div className="mx-auto mb-11 max-w-xl text-center">
            <div className="mono mb-2.5 text-xs font-semibold tracking-wide text-blue-600">{t.security.kicker}</div>
            <h2 className="mb-3 text-[32px] font-bold tracking-tight text-balance text-[#0f172a]">{t.security.head}</h2>
            <p className="text-[17px] leading-relaxed text-[#64748b]">{t.security.sub}</p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {t.security.items.map((s, i) => (
              <div key={s.title} className="rounded-2xl border border-[#eef1f4] bg-white p-5">
                <div className="mb-3.5 flex h-10 w-10 items-center justify-center rounded-[11px] bg-[#eef2ff] text-blue-600">
                  <SecurityIcon index={i} />
                </div>
                <div className="mb-1.5 text-[15px] font-bold text-[#0f172a]">{s.title}</div>
                <div className="text-[13.5px] leading-relaxed text-[#64748b]">{s.body}</div>
              </div>
            ))}
          </div>
        </AnimatedSection>
      ...
      <AnimatedSection className="mx-auto max-w-6xl px-4 py-20 sm:px-8">
        <div className="rounded-[28px] bg-gradient-to-br from-blue-600 to-blue-700 p-10 text-center text-white shadow-[0_24px_60px_rgba(37,99,235,0.35)] sm:p-14">
          <h2 className="mb-3 text-[32px] font-bold tracking-tight">{t.finalCta.head}</h2>
          <p className="mx-auto mb-6 max-w-[440px] text-[17px] text-blue-100">{t.finalCta.sub}</p>
          <Link
            href="/login"
...
function SecurityIcon({ index }: { index: number }) {
  const paths = [
    ...
    ["M4 7V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v2", "M2 9h20v10a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2z", "M6 13h4"],
