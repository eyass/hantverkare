...
export default async function QuotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: quote, error: quoteError } = await supabase
    .from("quotes")
    .select(
      "id, customer_description, status, subtotal_cents, vat_cents, total_cents, share_token, gallery_token, gallery_enabled, declined_at, decline_reason, assigned_to, deposit_percent, deposit_amount_cents, deposit_paid_at, ai_risk_flags, ai_risk_flags_acknowledged_at, ai_clarifying_questions, ai_clarifying_questions_resolved_at, viewed_at",
    )
    .eq("id", id)
    .single();
  if (!quote) {
  ...
  const { data: lineItems, error: lineItemsError } = await supabase
    ...
    .select(
      "id, description, quantity, unit, unit_price_cents, cost_cents, line_total_cents, position, price_list_item_id, item_type, quantity_reasoning, confidence, group_label",
    )
    .eq("quote_id", id)
    .order("position");
  if (lineItemsError) {
