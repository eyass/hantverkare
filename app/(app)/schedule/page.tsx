...
import { ScheduleGroups, type ScheduleGroup } from "./ScheduleGroups";
...
export default async function SchedulePage({
  ...
  function groupByDay(list: ScheduledJobRow[]): ScheduleGroup[] {
    ...
    return Array.from(groups.entries()).map(([key, jobsForDay]) => ({
      ...
      jobs: jobsForDay.map((job) => ({
        id: job.id,
        quote_id: job.quote_id,
        notes: job.notes,
        customerDescription: job.quotes?.customer_description ?? null,
        timeRangeLabel: formatTimeRange(job.scheduled_start, job.scheduled_end),
      })),
    }));
  }
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6 p-8">
      ...
      {upcomingGroups.length === 0 ? (
        ...
        <div className="flex flex-col gap-5">
          <ScheduleGroups groups={upcomingGroups} />
        </div>
      )}
      ...
      {pastGroups.length > 0 && (
        <details className="flex flex-col gap-3">
          ...
          <div className="mt-3 flex flex-col gap-5">
            <ScheduleGroups groups={pastGroups} />
          </div>
        </details>
      )}
    </div>
